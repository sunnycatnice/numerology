...
from myapi.serializers import numerologySerializer
...
@api_view(['GET', 'PUT', 'DELETE'])
def person_numerology(request, id, format=None):
    ...
    if request.method == 'GET':
        numerology = person.personnumerology_set.all()
        serializer = numerologySerializer(numerology, many=True)
        return Response({"numerology": serializer.data})

    elif request.method == 'PUT':
        numerology = person.personnumerology_set.all()
        serializer = numerologySerializer(numerology, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
